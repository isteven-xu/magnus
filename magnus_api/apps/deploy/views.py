...
def post_request_ext1(request):
    ...
    if error is None:
        ...
        form.magnus_version = Repository.make_magnus_version(deploy.id)
        if form.extra[0] == 'tag':
            if not form.extra[1]:
            ...
            form.magnus_version = repository.magnus_version
...
def post_request_ext1_rollback(request):
    ...
    if error is None:
        ...
        versions = list({x.magnus_version: 1 for x in requests}.keys())
        if req.magnus_version not in versions[:req.deploy.extend_obj.versions + 1]:
...
def post_request_ext2(request):
    ...
    if error is None:
        ...
        if DeployExtend2.objects.filter(deploy=deploy, host_actions__contains='"src_mode": "1"').exists():
            ...
            form.magnus_version = extra['path']
