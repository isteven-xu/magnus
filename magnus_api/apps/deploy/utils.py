...
from apps.deploy.helper import Helper, MagnusError
...
def dispatch(req, fail_mode=False):
    ...
    try:
        ...
        env = AttrDict(
            MAGNUS_APP_NAME=req.deploy.app.name,
            MAGNUS_APP_KEY=req.deploy.app.key,
            MAGNUS_APP_ID=str(req.deploy.app_id),
            MAGNUS_REQUEST_ID=str(req.id),
            MAGNUS_REQUEST_NAME=req.name,
            MAGNUS_DEPLOY_ID=str(req.deploy.id),
            MAGNUS_ENV_ID=str(req.deploy.env_id),
            MAGNUS_ENV_KEY=req.deploy.env.key,
            MAGNUS_VERSION=req.version,
            MAGNUS_BUILD_VERSION=req.magnus_version,
            MAGNUS_DEPLOY_TYPE=req.type,
...
def _ext1_deploy(req, helper, env):
    if not req.repository_id:
        rep = Repository(
            ...
            version=req.version,
            magnus_version=req.magnus_version,
            extra=req.extra,
            remarks='MAGNUS AUTO MAKE',
        ...
        with futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for t in futures.as_completed(threads):
                ...
                if exception:
                    ...
                    if not isinstance(exception, MagnusError):
...
def _ext2_deploy(req, helper, env):
    ...
    env.update({'MAGNUS_RELEASE': req.version})
    if req.version:
        for index, value in enumerate(req.version.split()):
            env.update({f'MAGNUS_RELEASE_{index + 1}': value})
    ...
    for action in host_actions:
        if action.get('type') == 'transfer':
            ...
            tar_gz_file = f'{req.magnus_version}.tar.gz'
    ...
    if host_actions:
        if req.deploy.is_parallel:
            ...
            with futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                for h_id in req.host_ids:
                    new_env = AttrDict(env.items())
                    t = executor.submit(_deploy_ext2_host, helper, h_id, host_actions, new_env, req.magnus_version)
                    t.h_id = h_id
                    threads.append(t)
                for t in futures.as_completed(threads):
                    exception = t.exception()
            ...
            while host_ids:
                ...
                new_env = AttrDict(env.items())
                try:
                    _deploy_ext2_host(helper, h_id, host_actions, new_env, req.magnus_version)
                    req.fail_host_ids.remove(h_id)
                except Exception as e:
                    helper.send_error(h_id, f'Exception: {e}', False)
                    for h_id in host_ids:
...
def _deploy_ext1_host(req, helper, h_id, env):
    ...
    env.update({'MAGNUS_HOST_ID': h_id, 'MAGNUS_HOST_NAME': host.hostname})
    extend = req.deploy.extend_obj
    extend.dst_dir = render_str(extend.dst_dir, env)
    extend.dst_repo = render_str(extend.dst_repo, env)
    env.update(MAGNUS_DST_DIR=extend.dst_dir)
    with host.get_ssh(default_env=env) as ssh:
        ...
        if code == 0:
            helper.send_error(host.id, f'检测到该主机的发布目录 {extend.dst_dir!r} 已存在，为了数据安全请自行备份后删除该目录，Magnus 将会创建并接管该目录。')
        if req.type == '2':
            ...
            command = f'cd {extend.dst_repo} && rm -rf {req.magnus_version} && tar xf {tar_gz_file} && rm -f {req.deploy_id}_*.tar.gz'
            helper.remote_raw(host.id, ssh, command)
            helper.send_step(h_id, 1, '\033[32m完成√\033[0m\r\n')

        # pre host
        repo_dir = os.path.join(extend.dst_repo, req.magnus_version)
...
def _deploy_ext2_host(helper, h_id, actions, env, magnus_version):
            ...
            if action.get('type') == 'transfer':
                if action.get('src_mode') == '1':
                    try:
                        ...
                        callback = helper.progress_callback(host.id)
                        ssh.put_file(os.path.join(REPOS_DIR, env.MAGNUS_DEPLOY_ID, magnus_version), dst, callback)
                    except Exception as e:
                        helper.send_error(host.id, f'Exception: {e}')
                    ...
                    sp_dir, sd_dst = os.path.split(action['src'])
                    tar_gz_file = f'{magnus_version}.tar.gz'
                    try:
                        callback = helper.progress_callback(host.id)
                    ...
                    command = f'mkdir -p /tmp/{magnus_version} && tar xf /tmp/{tar_gz_file} -C /tmp/{magnus_version}/ '
                    command += f'&& rm -rf {action["dst"]} && mv /tmp/{magnus_version}/{sd_dst} {action["dst"]} '
                    command += f'&& rm -rf /tmp/{magnus_version}* && echo "transfer completed"'
            else:
